gsagenie.run.analysis <- function(input, step1_status, step2_status, step3_status) {
  ...
  ##################################################################
  gst <- gsagenie.get.selected(input, step1_status, step3_status); 
  bth <- lapply(names(gst), function(nm) {
    c <- strsplit(nm, '; ')[[1]];
    g <- gsagenie.get.mapping(GENESET_HOME, c[1], c[2], spe, idf);
    m <- gsagenie.get.geneset(GENESET_HOME, c[1], c[2], spe); 
    list(g[gst[[nm]]], m[gst[[nm]], , drop=FALSE]); 
  });  
  cll <- rep(names(gst), sapply(gst, length));
  gst <- do.call('c', lapply(bth, function(x) x[[1]]));
  mta <- do.call('rbind', lapply(bth, function(x) x[[2]]));  
  mta$Collection <- cll;
  mta <- data.frame(ID=rownames(mta), Collection=cll, Name=mta$Name, URL=mta$URL, stringsAsFactors = FALSE);
  names(gst) <- rownames(mta) <- 1:length(gst); 
  step3_status$metadata <- mta;
  ##################################################################
  ...
  if (typ == 3) {
    ...
    if (ch2 == 2) { 
      bgd <- unique(unlist(gst, use.names=FALSE));
    } else if (ch2 == 1) {
      bgd <- readRDS(paste(GENE_HOME, '/', spe, '_just_id_', idf, '.rds', sep=''));
    } else {
      if (length(step2_status$background) > 0) bgd <- step2_status$background else 
        bgd <- unique(rownames(step2_status$table))
    }
  ...
  if (is.null(res)) step3_status$metadata <- NULL else {
    mta <- step3_status$metadata;
    step3_status$metadata <- mta <- mta[rownames(mta) %in% rownames(res), , drop=FALSE];
    step3_status$result   <- res[rownames(mta), , drop=FALSE];
  };
